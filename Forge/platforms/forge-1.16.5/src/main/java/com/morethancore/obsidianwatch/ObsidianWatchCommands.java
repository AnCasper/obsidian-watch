...
public final class ObsidianWatchCommands {
    ...
    public static void onRegisterCommands(RegisterCommandsEvent event) {
        ...
        dispatcher.register(Commands.literal("ow")
                ...
                .executes(context -> {
                    context.getSource().sendSuccess(mainMenu(), false);
                    return 1;
                })
                .then(Commands.literal("help")
                        ...
                        .then(Commands.literal("config")
                                .executes(context -> {
                                    context.getSource().sendSuccess(configHelp(), false);
                                    return 1;
                                }))
                        .then(Commands.literal("actions")
                                .executes(context -> {
                                    context.getSource().sendSuccess(actionsHelp(), false);
                                    return 1;
                                }))
                        .then(Commands.literal("exempt")
                                .executes(context -> {
                                    context.getSource().sendSuccess(exemptHelp(), false);
                                    return 1;
                                })))
                        ...
                        .executes(context -> {
                            if (service != null) {
                                ...
                                context.getSource().sendSuccess(ObsidianWatchText.join(
                                        ...
                                        ObsidianWatchText.line("Sync", "config and snapshot queued", TextFormatting.GREEN)
                                ), false);
                            } else {
                ...
                .then(Commands.literal("heartbeat")
                        .executes(context -> {
                            if (service != null) {
                                ...
                                context.getSource().sendSuccess(ObsidianWatchText.join(
                                        ObsidianWatchText.title("Obsidian Watch"),
                                        ObsidianWatchText.line("Heartbeat", "queued", TextFormatting.GREEN)
                                ), false);
                            } else {
                                sendUnavailable(context.getSource());
                            }
                ...
                .then(Commands.literal("config")
                        .executes(context -> {
                            if (service != null) {
                                ...
                                context.getSource().sendSuccess(ObsidianWatchText.join(
                                        ObsidianWatchText.title("Obsidian Watch"),
                                        ObsidianWatchText.line("Config", "remote config sync queued", TextFormatting.GREEN)
                                ), false);
                            } else {
                                sendUnavailable(context.getSource());
                            }
    ...
    private static ITextComponent mainMenu() {
    ...
    private static ITextComponent configHelp() {
        return ObsidianWatchText.join(
    ...
    private static ITextComponent actionsHelp() {
        return ObsidianWatchText.join(
    ...
    private static ITextComponent exemptHelp() {
        return ObsidianWatchText.join(
    ...
    private static void sendServiceMessage(CommandSource source, ServiceComponentSupplier supplier) {
        if (service == null) {
    ...
    private static void sendUnavailable(CommandSource source) {
        source.sendFailure(ObsidianWatchText.join(
                ObsidianWatchText.title("Obsidian Watch"),
                ObsidianWatchText.line("Status", "service unavailable", TextFormatting.RED)
        ));
    }

    private static void sendExemptionAdd(CommandSource source, String uuid) {
        if (service == null) {
            sendUnavailable(source);
            return;
        }
    ...
    private static void sendExemptionRemove(CommandSource source, String uuid) {
        if (service == null) {
            sendUnavailable(source);
            return;
        }
    ...
    private static void sendExemptionList(CommandSource source) {
        if (service == null) {
            sendUnavailable(source);
            return;
        }
    ...
    private static void sendTestLogResult(CommandSource source, String player, String action, String username) {
        if (service == null) {
            sendUnavailable(source);
            return;
        }
    ...
    private static void sendCheckResult(CommandSource source, String player, String username) {
        if (service == null) {
            sendUnavailable(source);
            return;
        }
    ...
    private interface ServiceComponentSupplier {
        ITextComponent get(ObsidianWatchService service);
    }
}
